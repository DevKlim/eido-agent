...
try:
    ...
    from agent.llm_interface import summarize_incident, recommend_actions
...
class EidoAgent:
    """
    The core agent responsible for processing EIDO messages (as dicts),
    ...
    def _extract_core_data_from_dict(self, eido_dict: Dict) -> Optional[ReportCoreData]:
        ...
        # --- Determine Primary Component (Incident or Call) ---
        # Prefer incidentComponent if it exists and is a non-empty list
        ...
        if isinstance(incident_comp_list, list) and incident_comp_list:
            primary_comp = incident_comp_list[0] # Use first incident component
            source_component_type = "IncidentComponent"
            if len(incident_comp_list) > 1:
                logger.warning(f"Msg {message_id}: Multiple IncidentComponents found ({len(incident_comp_list)}). Using first.")
        elif isinstance(call_comp_list, list) and call_comp_list:
            primary_comp = call_comp_list[0] # Fallback to first call component
            source_component_type = "CallComponent"
            logger.warning(f"Msg {message_id}: No IncidentComponent found. Using CallComponent for core data.")
            if len(call_comp_list) > 1:
                logger.warning(f"Msg {message_id}: Multiple CallComponents found ({len(call_comp_list)}). Using first.")
        else:
            logger.error(f"Msg {message_id}: Cannot extract core data - No IncidentComponent or CallComponent found or they are not lists.")
            return None

        if not isinstance(primary_comp, dict):
             logger.error(f"Msg {message_id}: Primary component ({source_component_type}) is not a dictionary. Cannot extract data.")
             return None
        ...
        # Use ReportCoreData validator to parse/convert timestamp
        timestamp = ReportCoreData.ensure_timezone_aware(timestamp_str or datetime.now(timezone.utc))

        # Incident Type (handle potential list)
        ...
        if not raw_incident_type and source_component_type == "CallComponent":
             raw_incident_type = primary_comp.get('callTypeCommonRegistryText', "Inferred Call") # Fallback for calls
        incident_type = None
        if isinstance(raw_incident_type, list) and raw_incident_type:
            incident_type = str(raw_incident_type[0]) # Take first item
        ...
        if isinstance(notes_list, list):
             for note_comp in notes_list:
                 if isinstance(note_comp, dict) and note_comp.get('noteText'):
                     ...
                     descriptions.append(f"[{timestamp_str_fmt}]{author_info}: {note_comp['noteText']}")
        ...
        if loc_ref_id and isinstance(location_components, list):
        ...
        elif isinstance(location_components, list) and location_components:
             logger.warning(f"Primary component ({source_component_type}) missing locationReference. Using first LocationComponent as fallback.")
             primary_location_comp = location_components[0] # Fallback to first location
        else:
             logger.debug(f"No LocationComponent list found in message {message_id}.")

        # Extract details from the found primary_location_comp (must be a dict)
        if isinstance(primary_location_comp, dict):
            ...
            if loc_ref_url and not location_address and not location_coords:
            ...
            for agency_comp in agency_components:
                 if isinstance(agency_comp, dict):
                    ...
                    if comp_id == agency_ref_id:
                        source_agency_name = agency_comp.get('agencyName', agency_ref_id) # Use name or ID
                        agency_found = True
