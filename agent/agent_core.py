...
class EidoAgent:
    ...
    def _resolve_ref_string_from_dict(self, ref_input: Any) -> Optional[str]:
        ...
        if isinstance(ref_input, str) and ref_input.startswith("$ref:"):
            ...
            if isinstance(first_item, dict) and '$ref' in first_item and isinstance(first_item['$ref'], str):
                ...
                if len(ref_input) > 1: logger.warning(f"Reference field contained multiple items, using first $ref: '{ref_id}'")
            elif isinstance(first_item, str) and first_item.startswith("$ref:"):
                ref_id = first_item.split(':', 1)[1]
                if len(ref_input) > 1: logger.warning(f"Reference field contained multiple strings, using first $ref: '{ref_id}'")
        return ref_id
    ...
    async def _attempt_geocode_and_update_store(self, text_to_geocode: str) -> Optional[Tuple[float,float]]:
        if not text_to_geocode or not isinstance(text_to_geocode, str) or not text_to_geocode.strip():
            logger.debug(f"Advanced Geocoding: Skipped due to empty or invalid text: '{text_to_geocode}'")
            return None
        logger.info(f"Advanced Geocoding: Attempting for text: '{text_to_geocode[:100]}...'")
        ...
        coords: Optional[Tuple[float,float]] = geocoding_result.get("coordinates")
        confidence: str = geocoding_result.get("confidence", CONFIDENCE_NONE)
        method: str = geocoding_result.get("method", "Unknown")
        for log_entry in geocoding_result.get("reasoning_log", []): logger.debug(f"AdvGeoLog: {log_entry}")
        if coords:
            logger.info(f"Advanced Geocoding successful for '{text_to_geocode[:100]}...': {coords}. Confidence: {confidence}, Method: {method}")
            if confidence in [CONFIDENCE_HIGH, CONFIDENCE_MEDIUM]:
                try:
                    ...
                    logger.error(f"Failed to cache advanced geocoding result for '{text_to_geocode[:100]}...': {e}", exc_info=True)
            else:
                logger.info(f"Advanced geocoding result for '{text_to_geocode[:100]}...' not cached due to confidence: {confidence}.")
            return coords
        else:
            logger.warning(f"Advanced Geocoding failed for '{text_to_geocode[:100]}...'. Method: {method}, Confidence: {confidence}.")
            return None

    def _extract_core_data_from_dict(self, eido_dict: Dict) -> Optional[ReportCoreData]:
        if not isinstance(eido_dict, dict):
            logger.error(f"Input to _extract_core_data_from_dict is not a dictionary (Type: {type(eido_dict)}).")
            return None
        
        message_id = eido_dict.get('eidoMessageIdentifier', eido_dict.get('$id', f"unknown_{str(uuid.uuid4())[:8]}"))
        logger.info(f"Extracting core data from EIDO Message Dict: {message_id}")
        ...
        for comp_type_key in ['incidentComponent', 'callComponent']:
            ...
            if isinstance(comp_list, list) and comp_list:
                primary_comp = next((comp for comp in comp_list if isinstance(comp, dict)), None)
                if primary_comp:
                    source_comp_type = comp_type_key.replace('Component', 'Component').capitalize()
                    break
        
        if not primary_comp:
            logger.error(f"Msg {message_id}: CRITICAL FAILURE - No suitable primary component found in EIDO dict.")
            return None

        incident_tracking_id = primary_comp.get('incidentTrackingIdentifier') or primary_comp.get('callTrackingIdentifier')
        ts_value = primary_comp.get('lastUpdateTimeStamp', eido_dict.get('lastUpdateTimeStamp'))
        timestamp = _ensure_timezone_aware(ts_value or datetime.now(timezone.utc))
        raw_incident_type = primary_comp.get('incidentTypeCommonRegistryText') or primary_comp.get('callTypeCommonRegistryText')
        incident_type = str(raw_incident_type[0]) if isinstance(raw_incident_type, list) and raw_incident_type else str(raw_incident_type)
        ...
        for comp_key in ['notesComponent', 'commentsComponent']:
            for item in eido_dict.get(comp_key, []):
                if isinstance(item, dict):
                    ...
                    if text: descriptions.append(text)
        full_description = "\n".join(descriptions) or primary_comp.get('incidentSummaryText') or "No description."
        ...
        primary_loc_comp = next((lc for lc in location_components if isinstance(lc, dict) and lc.get('$id') == loc_ref_id), None)
        if not primary_loc_comp and location_components:
            primary_loc_comp = next((lc for lc in location_components if isinstance(lc, dict)), None)
        ...
        if primary_loc_comp:
            ...
            if isinstance(loc_val, str) and loc_val.strip().startswith('<?xml'):
                ...
                location_address = format_address_from_components(addr_components) if addr_components else None
            else:
                location_address = primary_loc_comp.get('locationAddressText') or (loc_val if isinstance(loc_val, str) else None)
            
            location_narrative_for_geocoding = primary_loc_comp.get('locationNotes') or location_address
            if location_narrative_for_geocoding:
                coord_match = re.search(r'(-?\d{1,2}\.\d{3,})\s*[, ]\s*(-?\d{1,3}\.\d{3,})', location_narrative_for_geocoding)
                if coord_match: location_coords = (float(coord_match.group(1)), float(coord_match.group(2)))
                zip_match = re.search(r'\b(\d{5}(?:-\d{4})?)\b', location_narrative_for_geocoding)
                if zip_match: zip_code = zip_match.group(1)

        source_agency_name = eido_dict.get('sendingSystemIdentifier', "Unknown Source")
    ...
    async def _process_core_data(self, core_data: ReportCoreData) -> Dict:
        ...
        logger.info(f"Processing Core Data for Report ID: {core_data.report_id[:8]} (Orig. Msg ID: {message_id})")
        ...
        if not core_data.coordinates:
            ...
            if text_for_geocoding:
                ...
                if geocoded_coords:
                    ...
                    logger.info(f"Msg {message_id}: Advanced geocoding successful. Updated Coords: {core_data.coordinates}")
        
        try:
            active_incidents = await self.incident_store.get_active_incidents() 
            matched_id, score, reason = find_match_for_report(core_data, active_incidents)
        except Exception as e:
            logger.error(f"Msg '{message_id}': Error during incident matching: {e}", exc_info=True)
        ...
        if matched_id and (matched_incident := await self.incident_store.get_incident(matched_id)):
            ...
            logger.info(f"Msg '{message_id}': Matched to existing Incident {matched_id[:8]} (Score: {score:.2f}).")
            ...
            incident_to_process = PydanticIncident(incident_type=core_data.incident_type, status="Active")
            is_new = True
            logger.info(f"Msg '{message_id}': No match found. Creating new Incident {incident_to_process.incident_id[:8]}.")
            match_info = f"Created from Report {core_data.report_id[:8]} (ExtID: {core_data.external_incident_id or 'N/A'}, Reason: {reason})"
        
        incident_to_process.add_report_core_data(core_data, match_info=match_info)

        try:
            history = incident_to_process.get_full_description_history(exclude_latest=True) if not is_new else ""
            incident_to_process.summary = summarize_incident(history, core_data) or incident_to_process.summary
            incident_to_process.recommended_actions = recommend_actions(incident_to_process.summary, core_data) or incident_to_process.recommended_actions
        except Exception as e:
            logger.error(f"Incident {incident_to_process.incident_id[:8]}: Error during LLM interaction: {e}", exc_info=True)
        
        await self.incident_store.save_incident(incident_to_process)
        logger.info(f"Msg '{message_id}': Successfully processed. Report {core_data.report_id[:8]} -> Incident {incident_to_process.incident_id[:8]}.")
    ...
    async def process_report_json(self, json_data: Dict) -> Dict:
        ...
        message_id = json_data.get('eidoMessageIdentifier', json_data.get('$id', 'unknown'))
    ...
    async def process_alert_text(self, alert_text: str) -> Union[Dict, List[Dict]]:
        ...
        logger.info("--- Processing Raw Alert Text Block (New, Robust Pipeline) ---")
        ...
        for i, single_event_text in enumerate(event_texts):
            if not single_event_text.strip(): continue

            message_id = f"llm_parsed_{str(uuid.uuid4())[:8]}"
            logger.info(f"Event {i+1}: Calling LLM to extract structured data.")
            extracted_json_str = extract_eido_from_alert_text(single_event_text)

            if not extracted_json_str:
                logger.error(f"Event {i+1}: LLM did not return structured data.")
                results.append({"status": "Failed processing: LLM returned no data.", "message_id": message_id, "incident_id": None, "source_event_index": i + 1})
                continue
            
            try:
                extracted_data = json.loads(extracted_json_str)

                # FIX: Validate that the extracted data is a dictionary before proceeding.
                # This prevents the AttributeError if the LLM returns a JSON list or other non-dict type.
                if not isinstance(extracted_data, dict):
                    logger.error(f"Event {i+1} (Msg: {message_id}): LLM output was valid JSON but not a dictionary. Type: {type(extracted_data)}. Data: {extracted_json_str}")
                    results.append({"status": "Failed processing: LLM returned non-object JSON.", "message_id": message_id, "incident_id": None, "source_event_index": i + 1})
                    continue

                # Map LLM output keys to Pydantic model fields
                payload = {
                    "external_incident_id": extracted_data.get("external_id"),
                    "timestamp": extracted_data.get("timestamp_iso") or datetime.now(timezone.utc),
                    ...
                    "source": extracted_data.get("source_agency", "LLM-Parser-Agent"),
                    "original_document_id": message_id,
                    "original_eido_dict": {"source_text": single_event_text, "llm_output": extracted_data}
                }
                # Directly create and validate the ReportCoreData object
                core_data = ReportCoreData(**payload)
                ...
                logger.error(f"Event {i+1} (Msg: {message_id}): Pydantic validation failed for LLM output. Error: {e}", exc_info=True)
                print(f"\nDIAGNOSTIC: Pydantic validation failed for LLM output:\n{extracted_json_str}\nError: {e}\n")
                results.append({"status": "Failed processing: LLM output failed validation.", "message_id": message_id, "incident_id": None, "source_event_index": i + 1})
            except json.JSONDecodeError:
                logger.error(f"Event {i+1} (Msg: {message_id}): Failed to parse JSON from LLM response.")
                print(f"\nDIAGNOSTIC: LLM returned non-JSON string:\n{extracted_json_str}\n")
                results.append({"status": "Failed processing: LLM returned invalid JSON.", "message_id": message_id, "incident_id": None, "source_event_index": i + 1})
            except Exception as e:
                logger.error(f"Event {i+1} (Msg: {message_id}): An unexpected error occurred during raw text processing. Error: {e}", exc_info=True)
                results.append({"status": f"Failed processing: Unexpected error ({type(e).__name__}).", "message_id": message_id, "incident_id": None, "source_event_index": i + 1})
