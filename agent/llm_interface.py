...
def _get_llm_client(config: Dict[str, Any]) -> Optional[Any]:
    ...
    try:
        if provider == 'google':
            ...
            if cache_key in _client_cache: return _client_cache[cache_key]
    ...
    try:
        if provider == 'google':
            ...
            if response and hasattr(response, 'text') and response.text:
                 ...
                 failure_reason = getattr(response, 'prompt_feedback', 'Response was empty or malformed.')
                 logger.error(f"LLM call failed (Google). Reason: {failure_reason}")
                 return None

        elif provider in ['openrouter', 'local']:
            api_name = provider.capitalize()
            messages = [{"role": "system", "content": "You are an AI assistant. If JSON output is requested, provide ONLY the valid JSON object."}, {"role": "user", "content": prompt}]
            ...
            if response and response.choices and response.choices[0].message.content:
                ...
                return response.choices[0].message.content.strip()
                 ...
                 logger.error(f"LLM call failed or returned empty choices ({api_name}). Finish Reason: {response.choices[0].finish_reason if response.choices else 'N/A'}")
                 return None
                 
    except Exception as e:
        api_error_details = getattr(e, 'message', str(e))
        logger.error(f"Error calling LLM API ({provider} - {model_name}): {type(e).__name__}: {api_error_details}", exc_info=True)
        return None
    return None
...
def _clean_json_response(response_text: str) -> Optional[str]:
    ...
    try:
        ...
        logger.warning(f"Could not parse cleaned JSON candidate. Raw response prefix: {response_text[:100]}")
        return None
...
def fill_eido_template(template_str: str, scenario_desc: str) -> Optional[str]:
    if not template_str or not scenario_desc: return None
    ...
    prompt = PROMPTS['FILL_EIDO_TEMPLATE'].format(retrieved_context_str=retrieved_context_str, scenario_desc=scenario_desc, template_str=template_str)
...
def extract_eido_from_alert_text(alert_text: str) -> Optional[str]:
    ...
    retrieved_chunks = eido_retriever.retrieve_context(rag_query, top_k=3)
    retrieved_context_str = "\n\n**Relevant EIDO Schema Context:**\n---\n" + "\n---\n".join(retrieved_chunks) + "\n---" if retrieved_chunks else ""
    campus_context_hint = "\nThis alert may pertain to the UC San Diego (UCSD) campus. Consider common UCSD locations and landmarks." if any(keyword in alert_text.lower() for keyword in ["ucsd", "uc san diego", "geisel", "revelle"]) else ""
...
def split_raw_text_into_events(raw_text: str) -> Optional[List[str]]:
    ...
    if cleaned_response:
        try:
            ...
            logger.error(f"LLM response for splitting was not a valid JSON list of strings. Response: {cleaned_response[:200]}")
    return None

def geocode_address_with_llm(address_text: str) -> Optional[Tuple[float, float]]:
    if not address_text or not isinstance(address_text, str): return None
    config = _get_current_llm_config()
    if config.get('llm_provider') == 'none': return None
    ...
    if cleaned_response:
        try:
            ...
            logger.error(f"LLM response for geocoding was not valid JSON dictionary with lat/lon. Response: {cleaned_response[:200]}")
    return None

def extract_geolocatable_clues(narrative_text: str) -> Optional[Dict[str, Any]]:
    if not narrative_text or not isinstance(narrative_text, str): return None
    config = _get_current_llm_config()
    if config.get('llm_provider') == 'none': return None
    ...
    prompt = PROMPTS['EXTRACT_GEOLOCATABLE_CLUES'].format(geographic_context_prompt=geographic_context_prompt, narrative_text=narrative_text)
    response_text = _call_llm(prompt, is_json_output=True)
    cleaned_response = _clean_json_response(response_text)
    if cleaned_response:
        try:
            ...
            logger.error(f"LLM response for geo-clue extraction was not a valid JSON dictionary. Response: {cleaned_response[:200]}")
    return None