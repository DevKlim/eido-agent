...
class Settings(BaseSettings):
    ...
    log_level: Literal['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'] = Field("INFO", validation_alias='LOG_LEVEL')
    llm_provider: Literal['google', 'openrouter', 'local', 'none'] = Field("google", validation_alias='LLM_PROVIDER')
    ...
    def check_dependencies(self) -> 'Settings':
        ...
        if not self.database_url:
             settings_logger.error("DATABASE_URL is not set. The application cannot connect to the database.")
...
try:
    settings = Settings()
    settings_logger.info(f"Application settings loaded successfully. LLM Provider: {settings.llm_provider.upper()}")
except Exception as e:
    settings_logger.critical(f"CRITICAL ERROR: Failed to load application settings from environment: {e}", exc_info=False)
    settings_logger.warning("Falling back to default, safe settings. Application may not function correctly.")
