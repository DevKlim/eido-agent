# To ensure settings are loaded for logging level
...
class EidoSchemaRetriever:
    def __init__(self, index_path: str = INDEX_FILE_PATH):
        ...
        # Names of the schema components/chunks
    ...
    def _load_index(self):
        if not EMBEDDING_ENABLED:
            logger.warning(
                "Embedding service is disabled. EIDO Schema Retriever will not function.")
            self.is_ready = False
            return

        if not os.path.exists(self.index_path):
            logger.error(
                f"EIDO schema index file not found at: {self.index_path}")
            logger.error(
                "Please run the RAG indexer script first: python utils/rag_indexer.py")
            self.is_ready = False
            return
        ...
        try:
            ...
            if not self.index_data or "chunks" not in self.index_data or "embeddings" not in self.index_data:
                logger.error(
                    "Index file is invalid or missing required keys ('chunks', 'embeddings').")
                self.index_data = None
                self.is_ready = False
                return

            # Chunks are expected to be a list of dicts like {"name": "ComponentName", "text": "Formatted text"}
            self.chunk_names = [chunk.get(
                "name", f"UnnamedChunk_{i}") for i, chunk in enumerate(self.index_data["chunks"])]
            self.chunk_texts = [chunk.get("text", "")
            ...
            # current_model_dim is 0 if embedding model failed to load
            if index_dim != current_model_dim and current_model_dim != 0:
            ...
            if self.embeddings.shape[0] == 0 or (self.embeddings.shape[0] > 0 and self.embeddings.shape[1] != index_dim):
                logger.error(
                    f"Embeddings in index file have unexpected shape {self.embeddings.shape} or dimension mismatch with index_dim {index_dim}.")
                self.is_ready = False
                return

            if len(self.chunk_texts) != self.embeddings.shape[0]:
                logger.error(
                    "Mismatch between number of chunks and embeddings in index file.")
                self.is_ready = False
                return

            self.is_ready = True
            logger.info(
                f"EIDO Schema Retriever initialized successfully with {len(self.chunk_texts)} chunks from '{self.index_path}'.")

        except json.JSONDecodeError as e:
            logger.error(
                f"Error decoding JSON index file {self.index_path}: {e}")
            self.is_ready = False
        except Exception as e:
            logger.error(
                f"Error loading EIDO schema index: {e}", exc_info=True)
            self.is_ready = False

    def retrieve_context(self, query: str, top_k: int = 3) -> List[str]:
        if not self.is_ready:
            logger.warning(
                "Retriever is not ready (index file likely missing). Cannot retrieve context.")
            return []
        ...
        if self.embeddings is None or self.embeddings.shape[0] == 0:
            logger.error(
                "No embeddings loaded in retriever. Cannot retrieve context.")
            return []
        ...
        try:
            ...
            if query_embedding is None:
                logger.error(
                    "Failed to generate embedding for query, cannot retrieve context.")
                return []
            ...
            similarities = cosine_similarity(
                query_embedding_np, self.embeddings)[0]
            ...
            for rank, i in enumerate(top_k_indices):
                ...
                retrieved_chunks_details.append(
                    {"rank": rank + 1, "score": f"{score:.4f}", "name": name})
                retrieved_chunk_texts.append(text)

            logger.info(
                f"Retrieved {len(retrieved_chunk_texts)} context chunks for query '{query[:50]}...'. Top results: {retrieved_chunks_details}")
            return retrieved_chunk_texts

        except Exception as e:
            logger.error(
                f"Error during context retrieval for query '{query[:50]}...': {e}", exc_info=True)
            return []
        ...
        for tq in test_queries:
            ...
            if context:
                for i, chunk_text in enumerate(context):
                    print(
                        f"--- Chunk {i+1} ({eido_retriever.chunk_names[np.where(np.array(eido_retriever.chunk_texts) == chunk_text)[0][0]] if chunk_text in eido_retriever.chunk_texts else 'N/A'}) ---")
