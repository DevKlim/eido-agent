    ...
    async def geocode_narrative(self, narrative_text: str) -> Dict[str, Any]:
        ...
        if clues.get("explicit_addresses"):
            for address in clues["explicit_addresses"]:
                ...
                coords_llm_addr = geocode_address_with_llm(address)
                if coords_llm_addr:
        ...
        if named_entities:
            ...
            for entity in named_entities:
                ...
                sources_to_check = [
                    ("Local Store", local_geocoder.get_coordinates_from_local_store, 0.95),
                    ("UCSD Campus Geocoder", campus_geocoder.get_ucsd_coordinates, 0.90),
                    ("Nominatim POI", lambda name: nominatim_geocoder.get_coordinates(f"{name}, San Diego"), 0.75),
                    ("LLM Geocoder", geocode_address_with_llm, 0.65) # LLM is a fallback per-entity
                ]

                for source_name, geocode_func, confidence_score in sources_to_check:
                    try:
                        coords = await geocode_func(entity_name) if "async" in str(geocode_func) else geocode_func(entity_name)
                        if coords:
            ...
            # Select the best candidate from all entities processed
            if candidate_locations:
                ...
                if len(candidate_locations) > 1 and best_candidate['confidence_raw'] < 0.9:
                    confidence = CONFIDENCE_LOW # Multiple possibilities reduce confidence
                    reasoning_log.append("Reduced confidence due to multiple candidates.")
        ...
        if len(narrative_text.split()) > 3: # Avoid geocoding very short, non-address text
            reasoning_log.append("No strong entity matches found. Attempting to geocode full narrative with LLM as last resort.")
            coords_narrative_llm = geocode_address_with_llm(narrative_text)
            if coords_narrative_llm:
                reasoning_log.append(f"LLM geocoding of narrative result: {coords_narrative_llm}.")
                return {"coordinates": coords_narrative_llm, "confidence": CONFIDENCE_LOW, "method": "LLM (full narrative)", "extracted_clues": clues, "reasoning_log": reasoning_log}
