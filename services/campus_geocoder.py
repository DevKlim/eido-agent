...
class CampusGeocoder:
    def __init__(self, known_locations: Dict[str, Tuple[float, float]]):
        self.known_locations = {name.lower().strip(): coords for name, coords in known_locations.items()}
        logger.info(f"Campus Geocoder initialized with {len(self.known_locations)} known locations.")
    ...
    def get_coordinates_for_named_place(self, place_name: str) -> Optional[Tuple[float, float]]:
        ...
        if normalized_place_name in self.known_locations:
            ...
            logger.info(f"Found direct match for named place '{place_name}': {coords}")
            return coords

        # Partial match (e.g., if "Geisel" is mentioned for "Geisel Library")
        # This can be made more sophisticated with fuzzy matching if needed.
        for known_name, coords in self.known_locations.items():
            if normalized_place_name in known_name or known_name in normalized_place_name:
                # Add some heuristics to avoid overly broad matches if names are very short
                if len(normalized_place_name) > 3 or len(known_name) > 3 :
                    logger.info(f"Found partial match for '{place_name}' with known place '{known_name}': {coords}")
                    return coords
