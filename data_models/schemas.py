...
class ReportCoreData(BaseModel):
    ...
    external_incident_id: Optional[str] = Field(
        None, description="External ID like CAD number.")
    timestamp: datetime
    incident_type: Optional[str] = Field(
        None, description="Type of incident, e.g., 'Structure Fire'.")
    description: Optional[str] = Field(
        None, description="Narrative or notes of the report.")
    location_address: Optional[str] = Field(
        None, description="Full street address or intersection.")
    coordinates: Optional[Tuple[float, float]] = Field(
        None, description="Geographic coordinates (latitude, longitude).")
    zip_code: Optional[str] = Field(
        None, description="Postal/ZIP code associated with the report.")
    source: Optional[str] = Field(
        None, description="Reporting source, e.g., agency name, system ID.")
    ...
    def ensure_timezone_aware(cls, v):
        if isinstance(v, str):
            try:
                ...
                logger.warning(
                    f"Could not parse timestamp string '{v}': {e}. Using current UTC time.")
...
class Incident(BaseModel):
    ...
    locations: List[Tuple[float, float]] = Field(
        default_factory=list, description="Unique (lat,lon) coordinates associated.")
    addresses: List[str] = Field(
        default_factory=list, description="Unique textual addresses associated.")
    ...
    def add_report_core_data(self, core_data: ReportCoreData, match_info: Optional[str] = None):
        if not isinstance(core_data, ReportCoreData):
            logger.error(
                f"Invalid type added to Incident {self.incident_id[:8]}: {type(core_data)}")
            return

        logger.debug(
            f"Adding Report {core_data.report_id[:8]} (ExtID: {core_data.external_incident_id or 'N/A'}) to Incident {self.incident_id[:8]}.")
        self.reports_core_data.append(core_data)

        report_ts = core_data.timestamp  # Already timezone-aware UTC from ReportCoreData

        # If this is the first report, set created_at. Always update last_updated_at.
        ...
        if core_data.coordinates and isinstance(core_data.coordinates, tuple) and len(core_data.coordinates) == 2:
            try:
                coords_tuple = (float(core_data.coordinates[0]), float(
                    core_data.coordinates[1]))
                if coords_tuple not in self.locations:
                    self.locations.append(coords_tuple)
            except (ValueError, TypeError):
                logger.warning(
                    f"Invalid coords format in report {core_data.report_id[:8]}: {core_data.coordinates}")
    ...
    def get_full_description_history(self, exclude_latest: bool = False) -> str:
        ...
        sorted_reports = sorted(self.reports_core_data,
        ...
        for report in reports_to_include:
            if report.description:
                # Use isoformat() for a standard, timezone-aware string representation
